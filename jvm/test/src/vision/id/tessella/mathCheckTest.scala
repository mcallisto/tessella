...
class mathCheckTest extends FlatSpec with Checkers with CoordGenerators {
  ...
  it must "always return results smaller in absolute when the divisor is τ" in {
    check(forAll(arbitrary[Double]) {
      case small if small.abs < stdPrecision ⇒ mod(small, τ) <= τ
      case dividend                          ⇒ mod(dividend, τ) < τ
    })
  }
