...
class intersectionTest extends FlatSpec with ListUtils with TryUtils {
  ...
  "Two segments" can "not intersect" in {
    val s1: Segment2D = new Segment2D((-1.0, 4.0), (-1.0, 3.0))
    val s2: Segment2D = new Segment2D((-1.0, 1.0), (-1.0, 0.0))
    assert(!s1.isIntersecting(s2))
    assert(!s2.isIntersecting(s1))
    val s3: Segment2D = new Segment2D((-1.0, 3.0), (-1.0, 2.0))
    val s4: Segment2D = new Segment2D((-1.0, 1.0), (-1.0, 0.0))
    assert(!s3.isIntersecting(s4))
    assert(!s4.isIntersecting(s3))
    val s5: Segment2D = new Segment2D((6.5, 4.33012702), (6.0, 5.19615242))
    val s6: Segment2D = new Segment2D((5.0, 6.92820323), (4.5, 7.79422863))
  ...
  they can "be joined at one endpoint" in {
    val joind: Segment2D = new Segment2D((-1.0, 2.0), (2.0, 3.0))
    assert(diagonal.isJoining(joind))
  }

  they can "intersect" in {
    val s1: Segment2D = new Segment2D((-0.8660254, -0.5), (-0.8660254, 0.5))
    val s2: Segment2D = new Segment2D((-1.0, 0.0), (0.0, 0.0))
    val s3: Segment2D = new Segment2D((-0.8660254, 0.5), (-1.8660254, 0.5))
    val s4: Segment2D = new Segment2D((-1.5, 0.8660254), (-1.0, 0.0))
    val s5: Segment2D = new Segment2D((-1.5, 0.0), (0.5, 0.0))
    val s6: Segment2D = new Segment2D((-2.0, 0.0), (-1.0, 0.0))
    val s7: Segment2D = new Segment2D((-1.0, 0.0), (-1.0, -1.0))
  ...
  "A polygon" can "be tested if includes a point" in {
    val p = new Polygon(List((-1.0, 2.0), (1.0, 2.0), (0.0, 0.0)))
  ...
  it can "be tested if includes a segment" in {
    val p = new Polygon(List((0.0, 0.0), (4.0, 0.0), (4.0, 4.0), (2.0, 2.0), (0.0, 4.0)))
    assert(p.includes(new Segment2D((1.0, 1.0), (3.0, 1.0))))
    assert(p.includes(new Segment2D((1.0, 3.0), (3.0, 3.0))) === false)
  }
